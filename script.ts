...
const questions: { que: string; ans: string; opt: string[] }[] = [
  {
...
let totalQues = document.querySelector("#totalque") as HTMLSpanElement;
totalQues.innerText = questions.length.toString();

// Set anssheet to display none at the strat of quiz
let anssheet = document.querySelector(".ans-con") as HTMLDivElement;
...
function loadQuestion() {
  ...
  let que = document.querySelector(".que") as HTMLParagraphElement;
  que.innerHTML = `<span id="queno">${currIdx + 1}.</span> ${
    questions[currIdx].que
  }`;
  ...
  let optArr: string[] = questions[currIdx].opt;
...
let userAns: string[] = new Array(questions.length).fill("");
...
function calculateResult() {
  ...
  let container = document.querySelector(".que-container") as HTMLDivElement;
  ...
  let anslink = document.querySelector("#answersheet") as HTMLAnchorElement;
...
function showAnssheet() {
  let ansCon = document.querySelector(".ans-con") as HTMLDivElement;
  let anssheet = document.querySelector(".anssheet") as HTMLDivElement;
  ...
  questions.forEach((val, idx) => {
    anssheet.innerHTML += `<div style="margin-bottom:20px; border-bottom:1px solid #ddd; padding-bottom:12px;">
      ...
      <p id="userans">Your Answer: <span>${
        userAns[idx] === "" ? "No answer" : userAns[idx]
      }</span></p>
...
let totalSec: number = questions.length * 10;
...
setInterval(() => {
  ...
  let sec = totalSec % 60;
  let timecon = document.querySelector("#time") as HTMLParagraphElement;
